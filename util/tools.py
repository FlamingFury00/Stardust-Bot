    ...
    while i < struct.num_slices:
        ...
        if time_remaining > 0:
            ...
            ball_close = (ball_location - agent.foe_goal.location).magnitude() > (
                ball_location - agent.friend_goal.location
            ).magnitude()
                            ...
                            if forward_flag:
                                ...
                                if ball_location[2] > 600:
                                    aerial_attempt = aerial(
                                        ball_location - 92 * best_shot_vector,
                                        intercept_time,
                                        True,
                    ...
                    if not swapped:
                        ...
                        if in_field(ball_location - (250 * best_shot_vector), 1):
                            ...
                            if forwards:
                                ...
                                not forwards and ball_location[2] <= 280 and slope > 0.2
                            ):
                                hits[pair].append(
                                    jump_shot(
                                        ball_location,
...
def find_best_shot(agent, closest_foe):
    ...
    if len(shots["goal"]) > 0:
        score = (
            100.3
            - shots["goal"][0].intercept_time
            + agent.time
            + eta(closest_foe, shots["goal"][0].ball_location)[0] / 3
            - eta(agent.me, shots["goal"][0].ball_location)[0] / 3
        )
        if score > best_score:
            best_score = score
    ...
    if len(agent.friends) > 0:
        if len(shots["pass"]) > 0:
            score = 100 - shots["pass"][0].intercept_time + agent.time
            if score > best_score:
                best_shot = shots["pass"][0]
    # if len(shots["leftfield"]) > 0:
    #     score = (
    #         100
    #         - shots["leftfield"][0].intercept_time
    #         + agent.time
    #         + eta(closest_foe, shots["leftfield"][0].ball_location)[0] / 3
    #         - eta(agent.me, shots["leftfield"][0].ball_location)[0] / 3
    #     )
    #     if score > best_score:
    #         best_score = score
    ...
    #     score = (
    #         100
    #         - shots["rightfield"][0].intercept_time
    #         + agent.time
    #         + eta(closest_foe, shots["rightfield"][0].ball_location)[0] / 3
    #         - eta(agent.me, shots["rightfield"][0].ball_location)[0] / 3
    #     )
    #     if score > best_score:
    #         best_score = score
...
def find_best_save(agent, closest_foe):
    ...
    if len(saves["goal"]) > 0:
        score = (
            100
            - saves["goal"][0].intercept_time
            + agent.time
            + eta(closest_foe, saves["goal"][0].ball_location)[0] / 2
            - 1
            + distance_to_wall(saves["goal"][0].ball_location) / 1000
        )
        if score > best_score:
            best_score = score
